...
int main() {
    ...
    deleteFromHashTableTest(&map, "Ok");
    assert(sizeOfHashTableTest(&map) == 3);
    deleteFromHashTableTest(&map, "Test");
    assert(sizeOfHashTableTest(&map) == 2);
    assert(hasInHashTableTest(&map, "Hello"));
    assert(hasInHashTableTest(&map, "World"));
    assert(!hasInHashTableTest(&map, "Ok"));
    assert(!hasInHashTableTest(&map, "Test"));

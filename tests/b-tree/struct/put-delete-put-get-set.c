...
int main() {
    ...
    for (int i = 0; i < 1000; i++) {
        insertIntoBTreeTest(&map, TTT(i*i));
        assert(sizeOfBTreeTest(&map) == 1 + (size_t)i);
        isBTree(&map);
    }
    for (int i = 0; i < 1000; i++) {
        insertIntoBTreeTest(&map, TTT(i*i));
        assert(sizeOfBTreeTest(&map) == 1000);
        isBTree(&map);
    }
    for (int i = 0; i < 500; i++) {
        deleteFromBTreeTest(&map, TTT(i*i));
        assert(sizeOfBTreeTest(&map) == 999 - (size_t)i);
        isBTree(&map);
    }
    for (int i = 0; i < 500; i++) {
        deleteFromBTreeTest(&map, TTT(i*i));
        assert(sizeOfBTreeTest(&map) == 500);
        isBTree(&map);
    }
    for (int i = 0; i < 500; i++) {
        insertIntoBTreeTest(&map, TTT(i*i));
    ...
    for (int i = 0; i < 1000; i++) {
        assert(hasInBTreeTest(&map, TTT(i*i)));
        isBTree(&map);
    }
    deinitBTreeTest(&map);
    return 0;

...
int main() {
    ...
    deleteFromRBTreeTest(&map, "Ok");
    assert(sizeOfRBTreeTest(&map) == 3);
    deleteFromRBTreeTest(&map, "Test");
    assert(sizeOfRBTreeTest(&map) == 2);
    assert(hasInRBTreeTest(&map, "Hello"));
    assert(hasInRBTreeTest(&map, "World"));
    assert(!hasInRBTreeTest(&map, "Ok"));
    assert(!hasInRBTreeTest(&map, "Test"));
